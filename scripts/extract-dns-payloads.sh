...
#
# Example:
#   ./scripts/extract-dns-payloads.sh fuzz/corpus/parse_message testdata/samples/*.cap testdata/samples/*.pcap
#
set -euo pipefail
...
for pcap_file in "$@"; do
    ...
    # -e udp.payload: raw UDP payload (hex) for DNS packets
    count=0
    ...
    done < <(tshark -r "$pcap_file" -Y "dns" -T fields -e udp.payload 2>/dev/null || true)
